...
\label{Section:LinSystems}

\subsection{Introduction.  Consistent and Inconsistent Linear Systems}
\label{Subsec:LinSystems:Intro}
...
A \emph{linear equation} is an equation that can be written in the form
...
The numbers  $a_1,a_2,\ldots,a_n$ are called the \emph{coefficients} and the variables $x_1,x_2, \ldots, x_n$ the \emph{unknowns}.  The term $k$ is referred to as the \emph{constant term}.
\end{dfn}
...
A set of one or more linear equations is called a  \emph{system of linear equations}  (or \emph{linear system}, for short). In the case of $m$ linear equations in the variables $x_1,x_2,\ldots, x_n$ we speak of a \emph{system of $m$ equations in $n$ unknowns}.
...
A \emph{solution} of a linear system  is an ordered list of $n$  values  $(c_1,  c_2, \ldots, c_n)$, or, depending on the context, a vector
$
...
The \emph{solution set} or \emph{general solution} of a system is the set of all solutions.
\end{dfn}
...
A system of linear equations is \emph{consistent} if it has at least one solution.  Otherwise it is called \emph{inconsistent}. 
\end{dfn}
...
$$
    ...
    \left\{
\begin{array}{l} x_1 = 4 \\ x_2 = 2 \end{array} \right.
$$
...
\begin{stelsel}
        2x_1&-&5x_2&=&-2\\
        4x_1&-&7x_2&=& 2
    \end{stelsel}\right. \quad\Rightarrow\quad
     \left\{
\begin{stelsel}
        2x_1&-&5x_2&=&-2\\
            & &3x_2&=& 6
...
\subsection{Solving a linear system by elimination}
\label{Subsec:LinSystems:Elimination}
...
\label{Dfn:LinSystems:EquivalentSystems}
 Two systems of linear equations are called \emph{equivalent}  if they have the same set of solutions.
\end{dfn}
...
Of course if we multiply an equation with some nonzero constant, the solution set  also remains invariant.  This operation is called \emph{scaling}.
...
\begin{itemize}
\item Adding a multiple of an equation to another equation.
\item Scaling an equation with a nonzero scaling factor $c$.
\item Changing the order of the equations.
\end{itemize}
...
Example \ref{Ex:LinSystems:EliminationFirst}.  One example is by far not a proof, but the explanation given there can be generalized/formalized.  
...
The last few steps, working our way up from the last equation to successively find $x_3$, $x_2$, $x_1$, is sometimes referred to as \emph{back substitution}.
\end{ex}
...
\item If no such equation appears, the general solution can be found by back substitution:  starting from the last equation, we work our way upwards. 
\end{itemize}
...
\label{Subsec:LinSystems:AugmentedMatrix}
...
An $m \times n$  \emph{matrix} $A$ is an rectangular array of numbers  $a_{ij}, 1\leq i \leq m, 1 \leq j \leq n$.
...
It consists of  $m$ horizontal  \emph{rows}  of size $n$, or, equivalently, of $n$ vertical \emph{columns} of size $m$.
\end{dfn}
...
$$
\left[
  \begin{array}{c} 3 \\ -1 \\ 5   \end{array}\right]
$$
...
The \emph{augmented matrix} for a system of equations
...
is called the \emph{coefficient matrix}  of the system.  The column behind the bar contains the constant terms.
\end{dfn}
...
\begin{stelsel}
   ...
   \right.&\qquad&
   \begin{gmatrix}[b]
       1 &  3 & -2& \BARu & 4 \\  3 &  7 & -2& \BAR &  8 \\     2 & 10 & -9 & \BARl & 4
        \rowops
        \add[$-3$]{0}{1}
        \add[$-2$]{0}{2}
   \end{gmatrix} \\
    &\multicolumn{1}{c}{\Big\Updownarrow}  & \\
   \left\{
\begin{stelsel}
             x_1 & + & 3x_2 & -&2x_3 &=&  4 \\
                 & - & 2x_2 & +&4x_3 &=&  -4 \\
   ...
   \right.&\qquad&
   \begin{gmatrix}[b]
       1 &  3 & -2& \BARu & 4 \\  0& -2 &  4 &\BAR &  -4 \\     0 & 4 & -5 & \BARl & -4
        \rowops
        \mult{1}{-\frac12}
   \end{gmatrix}
   \\
...
\begin{stelsel}
   ...
   \right.&\qquad&
   \begin{gmatrix}[b]
       1 &  3 & -2& \BARu & 4 \\  0& 1 &  -2 & \BAR &  2 \\     0 & 4 & -5 & \BARl & -4
        \rowops
        \add[$-4$]{1}{2}
   \end{gmatrix}
   \\
...
\begin{stelsel}
   ...
   \right.&\qquad&
   \begin{gmatrix}[b]
       1 &  3 & -2& \BARu & 4 \\  0& 1 &  -2 & \BAR &  2 \\     0 & 0 & 3 & \BARl & -12
   \end{gmatrix}
\end{array}
$$
...
A matrix is  in \emph{row echelon form}  if it has the following two properties:

\begin{enumerate}[label=(\roman*)]
\item All non-zero rows are above all rows that contain only zeros.
\item Each non-zero row that is not the last row starts with fewer zeros than the rows below it.
\end{enumerate}

Such a matrix is also called a  \emph{row echelon matrix}.
\end{dfn}
...
In a similar manner we can define the concept of a \emph{column echelon matrix}.
...
A \emph{pivot} of a row in an echelon matrix is the first nonzero element of the row. Sometimes we also refer to it as the \emph{leading entry}.
\end{dfn}
...
$$
 ...
 \left[
\begin{array}{rrr}1 & 2 & 3 \\ 0 & 3 & 2 \\ 0 & 0 & 0 \end{array}  \right], \quad
 \left[
\begin{array}{rr}1 & 0 \\ 0 & 1 \\ 0 & 0 \\ 0 & 0 \end{array}  \right], \quad
 \left[
\begin{array}{rrrrr}1 & 1 & 0 &  2 &  0\\ 0 & 0 & 1 & 4 & 0\\ 0 & 0 & 0 & 0 & 1\end{array}  \right]
$$
...
$$
 ...
 \left[
\begin{array}{rrr}0 & 0 & 0 \\ 0 & 1 & 2 \\ 0 & 0 & 1 \end{array}  \right], \quad
 \left[
\begin{array}{rrr}1 & 0 & 0 \\ 0 & 1 & 1  \\ 0 &1 & 0 \end{array}  \right].
$$
...
$$
 ...
 \left[
\begin{array}{rrr}\fbox{1} & 2 & 3 \\ 0 & \fbox{3} & 2 \\ 0 & 0 & 0 \end{array}  \right], \quad
 \left[
\begin{array}{rr}\fbox{1} & 0 \\ 0 & \fbox{1} \\ 0 & 0 \\ 0 & 0 \end{array}  \right], \quad 
 \left[
\begin{array}{rrrrr}\fbox{1} & 1 & 0 &  2 &  0\\ 0 & 0 & \fbox{1} & 4 & 0\\ 0 & 0 & 0 & 0 & \fbox{1}\end{array}  \right].
$$
...
Note that from the second condition in Definition \ref{Dfn:LinSystems:EchelonForm} it follows that automatically all entries in the column below a pivot must be 0.
\end{rem}

Now have a look again at the derivation at the end of the previous subsection. We worked our way downwards through the rows to create zeros in the first columns, while keeping in mind that we did not change the solution set of the corresponding linear system. The process is called \emph{row reduction}.  The end point, from which we could start building the solution by back substitution, was an augmented matrix in echelon form!
...
The \emph{elementary row operations} that one can apply to a matrix are

\begin{enumerate}[label=(\roman*)]
\item Adding a multiple of a row to another row.
\item Multiplying a row by a non-zero number.  This is also referred to as \emph{scaling}.
\item Interchanging (or: swapping) two rows.
\end{enumerate}
...
Matrices that can be transformed into each other via row operations are called \emph{row equivalent}.
...
$$
   ...
   \end{gmatrix} \\
    &\sim&
  \begin{gmatrix}[b]
       1 & -1 & -1 & 2 & 3 \\  0 & 0 & 0 & 0 & 4 \\     0 & 0 & 2 & -1 & -5
        \rowops
        
        \swap{1}{2}
   \end{gmatrix} \\
  &\sim& \begin{gmatrix}[b]
       1 & -1 & -1 & 2 & 3  \\     0 & 0 & 2 & -1 & -5 \\  0 & 0 & 0 & 0 & 4
...
$$
  ...
  \begin{gmatrix}[b] 1 & 2 & 3 & 1\\ 1 & 4 & 7 & 3\\ 3 & 6 & 11 & 9
   \rowops
        \add[$-1$]{0}{1}
        \add[$-$3]{0}{2}
       \end{gmatrix} \sim 
    \begin{gmatrix}[b] 1 & 2 & 3 & 1\\ 0 & 2 & 4 & 2\\ 0 & 0 & 2 & 6
   \rowops
        \mult{1}{\frac12}
        \mult{2}{\frac12}
   \end{gmatrix}
    \quad \sim 
    \begin{gmatrix}[b] 1 & 2 & 3 & 1\\ 0 & 1 & 2 & 1\\ 0 & 0 & 1 & 3
   \rowops
        \add[$-3$]{2}{0}
        \add[$-2$]{2}{1}
   \end{gmatrix}
...
$$
 ...
 \sim   
   \begin{gmatrix}[b] 1 & 2 & 0 & -8\\ 0 & 1 & 0 & -5\\ 0 & 0 & 1 & 3
   \rowops
        \add[$-2$]{1}{0}
   \end{gmatrix}
   \,\sim
   \begin{gmatrix}[b] 1 &0  & 0 & 2\\ 0 & 1 & 0 & -5\\ 0 & 0 & 1 & 3
   \end{gmatrix}
$$
...
Apart from the second step, where two rows were scaled, in each step one pivot was used to make all elements right above and right below it equal to 0. In this way we move forward all the time to a matrix with more and more zeros \emph{in a structured way}. 
\end{rem}
...
\begin{itemize}
\item the pivots are all 1,  the simplest non-zero number
\item if we try to create more zeros, we can only do so in the fourth column. But then we we will lose one or more of the zeros in the first three columns. 
\end{itemize}
...
$$
    ...
    M = \left[
\begin{array}{rrr|r} 1 & 2 & 3 & 1\\ 1 & 4 & 7 & 3\\ 3 & 6 & 11 & 9
       \end{array}\right],
 $$
...
$$
    ...
    \left[
\begin{array}{rrr|r} 1 &0  & 0 & 2\\ 0 & 1 & 0 & -5\\ 0 & 0 & 1 & 3
       \end{array}\right]
$$
...
A \emph{reduced echelon matrix} or matrix \emph{in reduced echelon form} is an echelon matrix with the extra properties

\begin{enumerate}[label=(\roman*)]
\item All pivots are 1.
\item In a column with a pivot all other elements are 0.
\end{enumerate}
...
The big advantage of reduced echelon matrices, already hinted at in Remark (\ref{Rem:LinSystems:SolutionFromRREF}),  is the following:
...
In the solution as constructed according to the previous proposition the pivot variables are called the \emph{basic variables}. The other variables are called \emph{free variables}.
\end{dfn}
...
\begin{bmatrix} 1 & 0 & 2 & 0 & 3 & \BARu & 6\\ 
                   0 & 1 & -3 & 0 &-4 & \BAR  & 7\\ 
                   0 & 0 & 0 & 1 & 5 & \BARl & 8
   \end{bmatrix}
$$
...
The row reduction of the augmented matrix to  echelon form corresponds to the forward substitution as in examples (\ref{Ex:LinSystems:SolWithBacksubst1}) and  (\ref{Ex:LinSystems:SolWithBacksubst2}).  There we found the solution by back-substitution.  When the augmented matrix is reduced to  reduced echelon form
...
$$
  ...
  \begin{gmatrix}[b] 3 & 2 &1 &6&-2\\   0 & 2 & -2 &-3 & 1\\  0 & 0 & 0 &3 & 2
   \rowops
        \add[1]{2}{1}
        \add[$-2$]{2}{0}
       \end{gmatrix} \sim \quad
    \begin{gmatrix}[b] 3 & 2 &1 &0&-6\\   0 & 2 & -2 &0 & 3\\  0 & 0 & 0 &3 & 2
   \rowops
         \add[$-1$]{1}{0}
   \end{gmatrix}
    \quad 
...
\sim \quad
    \begin{gmatrix}[b] 3 & 0 &3 &0&-9\\   0 & 2 & -2 &-0 & 3\\  0 & 0 & 0 &3 & 2
   \rowops
        \mult{0}{\nicefrac13}
        \mult{1}{\nicefrac{1}{2}}
        \mult{2}{\nicefrac13}
   \end{gmatrix} \quad
 \sim   \quad
   \begin{gmatrix}[b] 1 & 0 &1 &0&-3\\   0 & 1 & -1 &-0 & 3/2\\  0 & 0 & 0 &1 & 2/3
...
$$
    ...
    \begin{gmatrix}[b]  2 & -1 & -1 & 2\\ 1 & 2 & 4 & 4\\ 4 & -2 & -4 & 6
      \rowops
        \mult{1}{2}
       \end{gmatrix} \quad\sim 
       \begin{gmatrix}[b]  2 & -1 & -1 & 2\\ 2 & 4 & 8 & 8\\ 4 & -2 & -4 & 6
      \rowops
        \add[$-1$]{0}{1}
        \add[$-2$]{0}{2}
       \end{gmatrix} 
...
$$
    ...
    \sim \begin{gmatrix}[b]  2 & -1 & -1 & 2\\ 0 & 5 & 9 & 6\\ 0 & 0 & -2 & 2
      \rowops
        \mult{2}{(-\nicefrac12)}
       \end{gmatrix} \quad\sim 
       \begin{gmatrix}[b]  2 & -1 & -1 & 2\\ 0 & 5 & 9 & 6\\ 0 & 0 & 1 & -1
      \rowops
        \add[$1$]{2}{0}
        \add[$-9$]{2}{1}
       \end{gmatrix} 
...
$$
    ...
    \sim \begin{gmatrix}[b]  2 & -1 & 0 & 1\\ 0 & 5 & 0 & 15\\ 0 & 0 & 1 & -1
      \rowops
        \mult{1}{(\nicefrac15)}
       \end{gmatrix} \quad\sim 
       \begin{gmatrix}[b]  2 & -1 & 0 & 1\\ 0 & 1 & 0 & 3\\ 0 & 0 & 1 & -1
      \rowops
        \add[$1$]{1}{0}
       \end{gmatrix} 
$$
...
$$
    ...
    \sim \begin{gmatrix}[b]  2 & 0 & 0 & 4\\ 0 & 1 & 0 & 3\\ 0 & 0 & 1 & -1
      \rowops
        \mult{0}{(\nicefrac12)}
       \end{gmatrix} \quad \sim 
       \begin{gmatrix}[b]  1 & 0 & 0 & 2\\ 0 & 1 & 0 & 3\\ 0 & 0 & 1 & -1
       \end{gmatrix}. 
$$
...
$$
    ...
    \begin{gmatrix}[b]  2 & -1 & -1 & 2\\ 1 & 2 & 4 & 4\\ 4 & -2 & -4 & 6
     \rowops
      \swap{0}{1}
       \end{gmatrix} \sim 
       \begin{gmatrix}[b]   1 & 2 & 4 & 4\\2 & -1 & -1 & 2\\4 & -2 & -4 & 6
      \rowops
        \add[$-2$]{0}{1}
        \add[$-4$]{0}{2}
       \end{gmatrix} 
...
$$
    ...
    \sim\begin{gmatrix}[b]  1 & 2 & 4 & 4\\ 0 & -5 & -9 & -6\\ 0 & -10 & -20 & -10
     \rowops
      \mult{2}{(-\nicefrac{1}{10})}
       \end{gmatrix} \sim 
       \begin{gmatrix}[b]  1 & 2 & 4 & 4\\ 0 & -5 & -9 & -6\\ 0 & 1 & 2 & 1
      \rowops
         \swap{1}{2}
       \end{gmatrix} 
$$
...
$$
    ...
    \sim\begin{gmatrix}[b]  1 & 2 & 4 & 4\\ 0 & 1 & 2 & 1 \\ 0 & -5 & -9 & -6
     \rowops
      \add[$-2$]{1}{0}
      \add[5]{1}{2}
       \end{gmatrix} \sim 
       \begin{gmatrix}[b]  1 & 0 & 0 & 2\\ 0 & 1 & 2 & 1 \\ 0 & 0 & 1 & -1
      \rowops
         \add[$-2$]{2}{1}
       \end{gmatrix} 
$$
...
$$
    ...
    \sim\begin{gmatrix}[b]  1 & 0 & 0 & 2\\ 0 & 1 & 0 & 3 \\ 0 & 0 & 1 & -1
       \end{gmatrix} , \text{\, the same outcome as before.} \rule{5em}{0ex}
$$
...
\begin{alg} Elimination method to solve a linear system. \\
Any system of linear equations can be solved as follows.

\begin{enumerate}[label=(\roman*)]
\item Write down the augmented matrix corresponding to the system.
\item Row reduce the augmented matrix to reduced echelon form.
\item If there is a pivot in the last column (the column 'behind the bar'), the system is inconsistent.
\item If the last column does not contain a pivot: write down the corresponding system of equations and express the variables in the pivot columns into the other variables (if any).  These other variables are free variables.
\end{enumerate}
...
A system of linear equations has either zero, or one, or infinitely many solutions.  In the case when there is exactly one solution, we speak of a \emph{unique} solution.
\end{thm}
...
This example stresses that the conclusion whether a linear system has zero, one or infinitely many solutions,  is basically a matter of the \emph{structure} of an echelon matrix that is equivalent to the augmented matrix of the system.  
Suppose the augmented matrices of three linear systems can be row reduced to the following matrices

$$

  E_1 = 
\begin{bmatrix} \blacksquare & \ast & \ast & \BARu & \ast \\  
                         0  & \blacksquare & \ast & \BAR & \ast \\
                         0  &  0 &\blacksquare & \BAR & \ast \\ 
                         0  & 0 & 0 & \BAR & 0 \\
                         0  & 0 & 0 & \BARl & 0  \end{bmatrix} , 
                   \quad
  E_2 =   
\begin{bmatrix} \blacksquare & \ast & \ast & \ast & \BARu & \ast \\  
                         0  & \blacksquare & \ast & \ast & \BAR & \ast \\
                         0  &  0 & 0 &\blacksquare & \BAR & \ast \\
                         0  & 0 & 0 &0 & \BARl & 0  \end{bmatrix}, 
                         \quad
  E_3 =   
\begin{bmatrix} \blacksquare & \ast &  \ast & \BARu & \ast \\  
                         0  & \blacksquare & \ast & \BAR & \ast \\
                         0  & 0 & \blacksquare & \BAR &  \ast \\
                         0  & 0 & 0 & \BAR & \blacksquare  \\
                         0  & 0  &0 & \BARl & 0  \end{bmatrix}                       
$$
...
can be seen as  a plane in $\R^3$.  The previous proposition tells us:  the intersection of $m$ planes in $\R^3$, where $m < 3$, cannot be a single point.
\end{rem}
