...
The line $\mathcal{L}$ of vectors orthogonal to a non-zero vector $\mathbf{n}$ in the plane.
```
...
The **orthogonal projection** _of a _vector_ $\mathbf{w}$ _onto_ a non-zero vector $\mathbf{v}$_ is the vector $\mathbf{\hat{w}} = c\mathbf{v} $ for which
...
for three non-zero vectors $\mathbf{v}, \,\mathbf{w}_1,\,\mathbf{w}_2$ in $\mathbb{R}^n$.
...
Moreover, for any non-zero vector $\mathbf{v}$,
the vector
...
Below we will show that it is possible to express the angle between _any_ two (non-zero) vectors into dot products. And use this to define the concept of angle in a general space $\R^n$.
...
Assume that $\mathbf{v}$ and $\mathbf{w}$ are non-zero vectors.
